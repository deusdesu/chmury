...
var poleek = "";
for (i = 1; i <= 81; i++) {
    poleek = poleek + '<input class="sudoku" id="id' + i + '" maxlength="1" type="text" />';
    if (i % 9 == 0) poleek = poleek + '<div style="clear:both;"></div>';
}
poleek = poleek + '<div style="clear:both;"></div>';
docID("pole_sudoku").innerHTML = poleek;
// wstawia bordery
for (i = 1; i <= 81; i++) {
...
docID("sprawdzenie").addEventListener("click", function(){
	if(sprawdz_czy_poprawne_sudoku() && !czy_pokazano_rozwiazanie){
		stoper_stop =false;
		zwyciestwo();
	}else{
...
if(!localStorage.getItem("sudoku_liczba")){
	localStorage.setItem( "sudoku_liczba",0 );
}	
...
function zapisz_wynik(){
	if(sprawdzPole("sudoku")){
		...
		localStorage.setItem("sudoku"+localStorage.getItem("sudoku_liczba"), JSON.stringify(wyniki));
		
		aktualna_liczba = localStorage.getItem("sudoku_liczba");
		localStorage.setItem( "sudoku_liczba",++aktualna_liczba );
		//reset mgły
 ...
 *
 * każdY poleek ma odpowiednie wartosci
 *
 * należY rozróżniać poleki wg pionu(Y), poziomu(X) wg małego 9 pólowego kwadratu (nazYwanY później kwadratem, lub CUBE)
 *
...
/**
 * [liczba_zapisanych_pol oznacza ile pól zostało zapisanych, zwiększa się za każdym użciem funkcji wstaw_value_do_obiektu_i_id]
 * @type {Number}
 */
var liczba_zapisanych_pol = 0;
var liczba_przed_funkcjami_lokalne; // sprawdza pojedyncze petle w petli "q" w funkcji send_numers()
var liczba_przed_funkcjami_glowne; // sprawdza czy zrobiono postep w petli "q" w funkcji send_numers()
...
function send_numers() {
    ...
    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! poprawić!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    for (q = 0; liczba_zapisanych_pol != 81; q++) {
        liczba_przed_funkcjami_glowne = liczba_zapisanych_pol;
        for (w = 0; liczba_zapisanych_pol != 81; w++) {
            liczba_przed_funkcjami_lokalne = liczba_zapisanych_pol;
            sprawdz_czy_istnieje_tylko_1_mozliwosc();
            jakie_l();

            if (liczba_przed_funkcjami_lokalne == liczba_zapisanych_pol) {
                // alert("nie udało się rozwiazać sudoku w 100%");
                break;
            }
        }

        for (e = 0; liczba_zapisanych_pol != 81; e++) {
            liczba_przed_funkcjami_lokalne = liczba_zapisanych_pol;
        ...
        */
        ...
        jakie_l();
        if (liczba_przed_funkcjami_glowne == liczba_zapisanych_pol) {
            // alert("nie udało się rozwiazać sudoku w 100%");
            break;
        }
    }
   // ostateczne_rozwiazanie_kwesti_sudoku();
    if( pierwszy_raz && liczba_zapisanych_pol != 81){
        pierwszy_raz = false;
        //element_losowy();
    }
    ...
    for (var a = 1; a <= 3; a++) {
        ...
        for (var nr1pozycja = 1; nr1pozycja <= 9; nr1pozycja++) { // pierwszy wymiar
            for (var nr2pozycja = 0; nr2pozycja <= 8; nr2pozycja++) { // drugi wymiar
                il_moz = obiekt[pozycja[nr1pozycja][nr2pozycja]].liczba_mozliwosci;
                for (var i = 0; i < il_moz; i++) {
                    ...
                    if(isNaN( numer ) ){
                        //alert(numer +" pozycja "+poz+" nr1pozycja "+nr1pozycja+" nr2pozycja "+nr2pozycja+" nr obiektu  "+pozycja[nr1pozycja][nr2pozycja]  +" source "+obiekt[pozycja[nr1pozycja][nr2pozycja]].toSource() +" liczba mozliwosci: "+il_moz );
                    }
...
function sprawdz_czy_istnieja_2_pary(tab) {
    ...
    for (var i = 1; i < length; i++) {
        ...
        pary_length = pary.length; // dł zwróconych par (w sumie to liczba)
        for (var u = 0; u < pary_length; u++) { // u to penta robiona tyle razy, ile jest par z danego tab[i]
            //   pary[u] to sa zwrócone pary, każda z liczb w parze(a sa ich 2) nalezy porownac z kazdym elementem w tab oprocz elementu, z ktorego para pochodzi. if liczba takich samych par wynosi UWAGA 1 to jest super, tego szukalismy ^_^
            for (var y = 1; y < length; y++) { // porównywanie ze wszystkimi elementami w tablicy heh
                ...
                if (czy_jest1 && czy_jest2) {
                    //zajebiscie, znalazlo pare heh ale ich liczba musi wynosić 1!!!!!
                    liczba_par++;
                }
            ...
            if (liczba_par == 2 && pochodzenie1 != pochodzenie2) {
                //alert( pary[u] );
...
function redukcja_przez_parę_cyfr_w_linii() {
    ...
    //kolejna próba, skan w poszukiwaniu liczba moz == 2 i sprawdzanie czy sa takie same

    var liczba_wyst2 = 0; // liczy liczba występowan .liczba_mozliwosci == 2
    var liczba_wyst3 = 0; // liczy liczba występowan .liczba_mozliwosci == 3
    var tablica_wynik2 = [];
    var tablica_wynik3 = [];
    var numer_pola;
    ...
    for (var a = 1; a <= 3; a++) {
        ...
        for (var nr1pozycja = 1; nr1pozycja <= 9; nr1pozycja++) { // pierwszy wymiar
            for (var nr2pozycja = 0; nr2pozycja <= 8; nr2pozycja++) { // drugi wymiar
                ...
                if (obiekt[numer_pola].liczba_mozliwosci == 2) { // pozycja[nr1pozycja][nr2pozycja] == numer
                    liczby[liczba_wyst2] = obiekt[numer_pola].jakie_liczby_mozna_wpisac;
                    tablica_wynik2[liczba_wyst2] = numer_pola;
                    liczba_wyst2++;
                }
                /*if (obiekt[numer_pola].liczba_mozliwosci == 3) {
                    tablica_wynik3[liczba_wyst3] = numer_pola;
                    liczba_wyst3++;
                }*/
            }
            // po przeliczeniu 1 pozycja następuje dalszy kod
            //
            // sprawdza czy .value_around sa takie same dla dwoch pól (dla liczba_wyst2 == 2)
                        ...
                        if (id_2_liczby) {
                            ...
                            for (var u = 0; u <= 8; u++) {
                                if (pozycja[nr1pozycja][u] != id_1_liczby && pozycja[nr1pozycja][u] != id_2_liczby && obiekt[pozycja[nr1pozycja][u]].value_around[pierwsza_liczba] == false) { // warunki: nie moze to byc id_1_liczby i id_2_liczby, obiekt[ pozycja[nr1pozycja][u] ].value_around[pierwsza_liczba] == false
                                    ...
                                    obiekt[pozycja[nr1pozycja][u]].liczba_mozliwosci--;
                                }
                ...
                if (czy_jest_rowne) { // jeżeli czy_jest_rowne nie zostało zmienione oznacza to, value_aroud sa takie same!!
                    ...
                    for (var p = 0; p <= 8; p++) { // sprawdzanie i przydział
                        ...
                        if (obiekt[pozycja[nr1pozycja][p]].value_around[jakie_to_liczby[0]] == false  && pozycja[nr1pozycja][p] != tablica_wynik2[0] && pozycja[nr1pozycja][p] != tablica_wynik2[1]) { // wiele warunków XD, oba .value_around msuza być false i nie moga to byc wybrane wczesniej ( tablica_wynik2[] ) liczby
                            ...
                            obiekt[pozycja[nr1pozycja][p]].liczba_mozliwosci --;
                        }
            ...
            //reset
            jakie_to_liczby = [];
            liczba_wyst2 = 0; // reset! XD
            liczba_wyst3 = 0; // reset! XD
            tablica_wynik2 = [];
            //tablica_wynik2[1] = 0;
            tablica_wynik3[0] = 0;
            tablica_wynik3[1] = 0;
...
function sprawdz_czy_istnieje_tylko_1_mozliwosc() { //
    for (var i = 1; i <= 81; i++) {
        if (obiekt[i].liczba_mozliwosci == 1) {
            for (var u = 1; u <= 9; u++) {
...
function redukcja_przez_wykluczenie() { // funkcja sprawdzajaca czy liczba false jest w przedziale [2,3], jezeli tak to zprawdza, czy maja taki sam X lub Y, jezeli tak to w tym X lub Y liczba y nie moze się powtorzyc
    ...
    //          ustawia dla wszytkich(oprócz wczeniejszych obiektów) obiektów z X == X obiektów .value_around[x] = true i zmiejsza liczba_mozliwosci--;

    //zmienne
    var liczba_wystepowan = 0;
        ...
        for (var nr1cube = 1; nr1cube <= 9; nr1cube++) { // nr1cube - pierwszo wymiarowy numer w CUBE[nr1cube][]
            for (var nr2cube = 0; nr2cube <= 8; nr2cube++) { // nr2cube - drugo wymiarowy numer w CUBE[][nr2cube]
                ...
                if (obiekt[CUBE[nr1cube][nr2cube]].value == "" && obiekt[CUBE[nr1cube][nr2cube]].value_around[szukana] == false) { // sprawdza czy pole jest puste i czy 'szukana' jest mozliwa do wpisania
                    ...
                    nr++;
                    if (liczba_wystepowan > 3) { //if przekroczy szukane_tablica resetuje liczba_wystepowan i idzie skanować następnego CUBE
                        nr2cube = 8;
                        liczba_wystepowan = 0;
                        nr = 0;
                ...
                if (obiekt[szukane_tablica[0]].X == obiekt[szukane_tablica[1]].X) { //sprawdza czy .X sa równe (dla 2)
                    ...
                    for (var i = 0; i <= 8; i++) {
                        if (obiekt[pozycjaX[number_of_position][i]].value == '' && obiekt[pozycjaX[number_of_position][i]].value_around[szukana] == false && pozycjaX[number_of_position][i] != szukane_tablica[0] && pozycjaX[number_of_position][i] != szukane_tablica[1]) { // warunki: .value jest puste, obiekt ma .value_around ustawione na false, pozycja jest różna od liczb w "szukane_tablica"
                            ...
                            obiekt[pozycjaX[number_of_position][i]].liczba_mozliwosci--;
                            sprawdz_czy_jest_tylko_1_true_w_pozycji();
                            sprawdz_czy_istnieje_tylko_1_mozliwosc();
                        }
                    }
                ...
                if (obiekt[szukane_tablica[0]].Y == obiekt[szukane_tablica[1]].Y) { //sprawdza czy .X sa równe (dla 2)
                    ...
                    for (var i = 0; i <= 8; i++) {
                        if (obiekt[pozycjaY[number_of_position][i]].value == '' && obiekt[pozycjaY[number_of_position][i]].value_around[szukana] == false && pozycjaY[number_of_position][i] != szukane_tablica[0] && pozycjaY[number_of_position][i] != szukane_tablica[1]) {
                            ...
                            obiekt[pozycjaY[number_of_position][i]].liczba_mozliwosci--;
                            sprawdz_czy_jest_tylko_1_true_w_pozycji();
                            sprawdz_czy_istnieje_tylko_1_mozliwosc();
...
function v2_zmianaX(iks, id_skrot, y) {
    for (var il_nr = 0; il_nr <= 8; il_nr++) {
        ...
        if (id_ob_do_zmiany != tab_z_false[0] && id_ob_do_zmiany != tab_z_false[1]) {
            ...
            //obiekt[id_ob_do_zmiany].liczba_mozliwosci--;
...
function wstaw_value_do_obiektu_i_id(id, wstawiona_wartosc) {
    if (obiekt[id].value != wstawiona_wartosc && tab_liczba_tylko_raz[id] == false) {
        ...
        // wg wsadzonych wartosci do obiektów/documentów zmienia .value_around i .liczba_mozliwosciobiektów wokół, zlepek z funkcji
        numerX = obiekt[id].X;
        numerY = obiekt[id].Y;
        numerCUBE = obiekt[id].CUBE;
        for (var i = 0; i <= 8; i++) {
            // szuka liczb dla pozycjiX, za pierwszym razem zmiejsza .liczba_mozliwosci o 1
            if (obiekt[pozycjaX[numerX][i]].value_around[wstawiona_wartosc] == false) {
                obiekt[pozycjaX[numerX][i]].liczba_mozliwosci--;
            ...
            if (obiekt[pozycjaY[numerY][i]].value_around[wstawiona_wartosc] == false) {
                obiekt[pozycjaY[numerY][i]].liczba_mozliwosci--;
            ...
            if (obiekt[CUBE[numerCUBE][i]].value_around[wstawiona_wartosc] == false) {
                obiekt[CUBE[numerCUBE][i]].liczba_mozliwosci--;
                obiekt[CUBE[numerCUBE][i]].value_around[wstawiona_wartosc] = true;
            }
        }
        liczba_zapisanych_pol++;
...
function okresl_obiekty() {
    for (var i = 1; i <= 81; i++) {
        obiekt[i] = {
            ...
            id: i,
            liczba_mozliwosci: 9, // zmiejsza się ze każdym razem, gdy zostawnie przyznana wartosc true dla "obiekt[i].value_around"
